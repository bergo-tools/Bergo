...
func selectProvider() *ProviderConfig {
...
// inputAPIKey 输入API密钥
func inputAPIKey(provider *ProviderConfig) string {
...
// selectModel 选择主模型
func selectModel(provider *ProviderConfig) string {
...
// createConfig 创建配置对象
func createConfig(provider *ProviderConfig, apiKey string, mainModel string) map[string]interface{} {
	cfg := make(map[string]interface{})

	// 基本配置
	cfg["debug"] = false
	cfg["language"] = "chinese"
	cfg["line_budget"] = 1000
	cfg["compact_threshold"] = 0.8

	// 设置主模型
	cfg["main_model"] = mainModel

	// 根据主模型类型设置berag模型
	beragModel, err := GetNonReasoningModel(mainModel)
	if err != nil {
		pterm.Warning.Print(locales.Sprintf("Failed to get berag model, using main model: %v\n", err))
		beragModel = mainModel
	}
	cfg["berag_model"] = beragModel
	cfg["berag_extract_model"] = beragModel

	// 设置API密钥
	cfg[provider.APIKeyField] = apiKey
...
func RunInitWizard() {
	...
	cfg := createConfig(provider, apiKey, model)
